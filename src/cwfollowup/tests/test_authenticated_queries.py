...
class TestQueriesWithAuthenticatedUser(CwFollowupTestCase):
    ...
    @mock.patch('cwfollowup.models.request_file_download_id', side_effect=request_file_download_id_mock)
    def test_cwfollowup_result_files(self, request_file_list, request_file_download_id_mock):
        """
        CwFollowupResultFiles query should return a file object.
        """
        job = CwFollowupJob.objects.create(
            user_id=self.user.id,
            name="Test1",
        ...
        response = self.client.execute(
            f"""
            query {{
                cwfollowupResultFiles (jobId: "{global_id}") {{
                    files {{
                        path
                        isDir
                        fileSize
